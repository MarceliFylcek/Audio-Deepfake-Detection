...
"""
Implementation of lpc transform using librosa 
"""
class LPC:
    def __init__(
        self,
        ...
        # Create the lpc
        self.lpc = torch.tile(torch.tensor(lpc_transform), ((self.waveform.shape[1]//self.hop_length),1))

        if db_amplitude:
            self.lpc = amplitude_to_DB(
                self.lpc,
    ...
    def _create_plot(self):
        """Creates a Linear Prediction Coefficients plot"""
        ...
        # Plot spectogram
        lpc_plot = ax.imshow(
            self.lpc, aspect="auto", origin="lower", cmap="magma"
        )

        # Set label names and title
        ax.set_xlabel("Frames")
        ax.set_ylabel("Coefficients")
        ax.set_title("Linear Prediction Coefficients")

        # Set fig type as colorbar
        fig.colorbar(lpc_plot, format="%+2.0f", cmap="magma")

        return fig, ax

    def display(self):
        """Creates and displays Linear Prediction Coefficients plot"""
